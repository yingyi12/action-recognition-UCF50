...
def collate_fn(batch):
    ...
    padded_time = torch.nn.utils.rnn.pad_sequence(time,batch_first=True,padding_value=1)
...
class DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        ...
        # df_train = pd.read_pickle('/nas/lrz/tuei/ldv/studierende/data/video/random/train11-random.pkl')
        # df_val = pd.read_pickle('/nas/lrz/tuei/ldv/studierende/data/video/random/val11-random.pkl')
        # df_test = pd.read_pickle('/nas/lrz/tuei/ldv/studierende/data/video/random/test11-random.pkl')
        

        X_train, y_train, X_train_time= prepare_dataset(df_train)
        X_val, y_val, X_val_time = prepare_dataset(df_val)
        X_test, y_test, X_test_time = prepare_dataset(df_test)
        
        if stage == 'fit' or stage is None:
            self.X_train = X_train
            self.y_train = y_train
            self.X_train_time = X_train_time
            self.X_val = X_val
            self.y_val = y_val
            self.X_val_time = X_val_time

        if stage == 'test' or stage is None:
            self.X_test = X_test
            self.y_test = y_test
            self.X_test_time = X_test_time
        

    def train_dataloader(self):
        train_dataset = Dataset(self.X_train,self.y_train, self.X_train_time)
    ...
    def val_dataloader(self):
        val_dataset = Dataset(self.X_val,self.y_val, self.X_val_time)
    ...
    def test_dataloader(self):
        test_dataset = Dataset(self.X_test,self.y_test, self.X_test_time)
...
class TimeGatedLSTM(pl.LightningModule):
    ...
    def on_validation_epoch_end(self):
        if self.current_epoch == self.trainer.max_epochs - 1:
            ...
            plt.savefig('/nas/lrz/tuei/ldv/studierende/data/video/keyframe-result/keyframe-tglstm/42/myfigure.png')
...
csv_logger = CSVLogger('/nas/lrz/tuei/ldv/studierende/data/video/keyframe-result', name='keyframe-tglstm', version='42'),
